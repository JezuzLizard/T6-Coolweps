...
main()
{
	replacefunc( maps\mp\zm_transit_gamemodes::init, ::init_gamemodes );
	...
	if ( isdefined( level.add_struct_gamemode_location_funcs[ gametype ] ) && isdefined( level.add_struct_gamemode_location_funcs[ gametype ][ location ] ) )
	{
	...
	{
		level.ctsm_disable_custom_perk_locations = getDvarInt( "ctl_no_extra_perks" ) == 1;
	}
	if ( !isDefined( level.ctl_disable_mystery_box ) )
	{
		...
		{
			level thread kill_start_chest_thread();
		}
	}
